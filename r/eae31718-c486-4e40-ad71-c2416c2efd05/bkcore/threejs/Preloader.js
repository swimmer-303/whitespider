...
bkcore.threejs.Preloader = function (opts) {
  ...
  try {
    this.renderer = new THREE.WebGLRenderer({ clearColor: 0xffffff });
  } catch (e) {
  ...
  try {
    ...
    console.error("Orthographic camera not supported, falling back to PerspectiveCamera.");
...
bkcore.threejs.Preloader.prototype.render = function () {
  ...
  this.stage.rotation.x +=
    (this.y - this.stage.rotation.x) * 0.3;
...
/**
 * Sets the preloader's ratio
 *
 * @param {number} r - The new ratio
 */
bkcore.threejs.Preloader.prototype.setRatio = function (r) {
  this.ratio = r;
};
