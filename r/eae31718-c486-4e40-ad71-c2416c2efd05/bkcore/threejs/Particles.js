 ...
 * @param {Object{max: number, spawnRate: number, spawn: THREE.Vector3, velocity: THREE.Vector3, randomness: THREE.Vector3, force: THREE.Vector3, spawnRadius: THREE.Vector3, life: number, friction: number, color: THREE.Color, color2: THREE.Color, tint: number, texture: THREE.Texture, size: number, blending: number, depthTest: boolean, transparent: boolean, opacity: number}} opts
 */
...
/**
 * Documentation for the build method
 */
...
/**
 * Documentation for the getParticle method
 * @returns {bkcore.threejs.Particle}
 */
bkcore.threejs.Particles.prototype.getParticle = function()
{
    if (this.pool.length > 0) {
        return this.pool.pop();
    }
    return new bkcore.threejs.Particle();
}
...
{
    var emitable = Math.min(count, this.max - this.buffer.length);
 ...
 * @param {bkcore.threejs.Particle} p
 */
bkcore.threejs.Particles.prototype.spawnParticle = function(p)
{
    p.available = false;
...
/**
 * Generates a random vector in the range [-1, 1]
 ...
 * @param {bkcore.threejs.Particle} p
 */
bkcore.threejs.Particles.prototype.resetParticle = function(p)
{
    p.position.set(0,-100000,0);
...
/**
 * Resizes the particle system to a new maximum number of particles
 * @param {number} newMax
 */
bkcore.threejs.Particles.prototype.resize = function(newMax)
{
    this.max = newMax;
    this.geometry = new THREE.Geometry();
    this.geometry.dynamic = true;
